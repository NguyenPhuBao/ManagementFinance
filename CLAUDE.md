...
| Việc thuộc backend | **`docs/superpowers/backend/CAN-LAM/README.md`** — **cửa vào duy nhất**. Nay còn **sáu** mục: **(D)** của `/sync/push` (`threshold_warning_percent` bị ép về 0), **cột màu danh mục**, **hai mục hoá đơn** (`transaction.Idbill` + `bill.Previous_bill_id`, rồi `bill.Auto_pay` + chốt chặn trả hai lần), **`Pay_status = 'Skipped'`** (không cần migration, client chỉ chờ **một câu xác nhận**), và **`bill.Anchor_day`** (thêm 2026-09-08 — client đã làm xong ở DB v18 nhưng cột đang cục bộ). Đếm theo **mục 2 của README ấy**; các bản tóm tắt trước từng ghi "bốn" rồi "năm" vì đếm theo trí nhớ. README chia việc theo *client đã có tính năng ấy chưa* rồi mới tới danh sách còn lại ở mục 2 — đọc theo đúng thứ tự ấy. Tài liệu **đã đóng** nằm ở **`DA-XONG/`** (16 tệp, kèm ghi chú đóng bằng cách nào) — mở khi cần biết *vì sao* lược đồ có hình dạng hôm nay, không phải khi tìm việc. Thư mục cha chỉ còn mục lục và ba tệp bối cảnh. Bảng trạng thái đầy đủ ở mục 14 `docs/PROJECT_CONTEXT.md` |
...
```bash
# Test (chạy từ src/Client-app) — hiện 1466/1466 pass, ~80 giây
flutter test
