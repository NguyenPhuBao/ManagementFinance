...
| Việc thuộc backend | `docs/superpowers/backend/README.md` — **mục 0 là thứ tự thi công chín bước chia ba nhóm**, đọc trước; mục 2 là trạng thái từng tài liệu. Hiện **8 tài liệu còn việc**. Bước 1 là thứ đang gây hại ngay: **Socket.io không xác thực + `io.emit` toàn cục** (`2026-09-04-ocr-classify-review.md`). Sau đó là `2026-09-04-backend-idempotent-delete.md` (ba lỗ hổng của `/sync/push`, một trong đó chặn hẳn ngân sách "Ngày cụ thể") và `CATEGORY_KEYWORD_SYNC.md` (lỗ hổng phân quyền). Bảng trạng thái đầy đủ ở mục 14 `docs/PROJECT_CONTEXT.md` |
