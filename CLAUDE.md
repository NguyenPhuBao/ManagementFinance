...
1. **Chỉ sửa `src/Client-app`.** Không đụng `src/Backend` trừ khi được cho phép rõ ràng trong chính yêu cầu đó. Cần backend làm gì thì **viết tài liệu** vào `docs/superpowers/backend/`.
   ...
   - ✅ **CSDL dev trên máy này đã trở về đúng lược đồ chuẩn** (2026-09-10). Sự việc trên từng để lại `wallet."Status"` ở `varchar(16)` và một dòng thừa trong `_prisma_migrations`; người dùng yêu cầu **đích danh** chạy hai lệnh hoàn tác, và đo lại sau đó: cột về `varchar(7)`, lịch sử migration còn 3 dòng, bốn `chk_wallet_*` còn đủ, số hàng không đổi. Nên từ đây **đẩy `'Inactive'` lên lại vỡ như cũ** — G28 vẫn mở, đúng như tài liệu xin. Diễn biến ở mục **3b** của tài liệu vừa dẫn.
