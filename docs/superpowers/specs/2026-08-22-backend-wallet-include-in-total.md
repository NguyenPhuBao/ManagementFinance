...
✅ **Đã trả lời 2026-09-09:** `getWalletsByAccount` dùng `prisma.wallet.findMany` nên trả **mọi cột**, kể cả cột này — phía backend không thiếu gì. Nửa thiếu nằm ở **client**: mapper kéo về trong `sync_engine.dart` không đọc khoá ấy. Đã sửa, kèm chốt khoá-vắng-mặt vì `doiSangBool(null)` trả `false` (tức đọc thẳng sẽ *tắt* cờ chứ không *giữ* nó).
