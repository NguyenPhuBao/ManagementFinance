...
**Status: ĐÃ THI CÔNG** (soát lại 2026-09-08). Mã nằm ở
`src/Client-app/lib/features/category/`, cụ thể
`data/repositories/category_management_repository.dart`; phần danh mục có **125
test** trên 10 tệp. Dòng cũ ở đây ghi *"implementation has not started"* và đã
sai suốt nhiều tuần.

⚠️ Thiết kế bên dưới là **bản gốc**, và một phần đã bị vượt qua: quy tắc trùng
tên nay bỏ `classify` khỏi khoá và tính cả danh mục mặc định, còn danh mục mặc
định đã thành **bản sao riêng từng tài khoản** (2026-09-07). Lý lẽ hiện hành ở
`docs/CATEGORY_RATIONALE.md` và quy tắc 7–8 `CLAUDE.md`; đọc chúng trước, tài
liệu này chỉ để biết ý định ban đầu.
...
| Thêm giao dịch - Bàn phím neo đáy (2026-09-19) | `acf6f17e65b84132ae6b18bba16a606a` | Bố cục đang chạy: số tiền cố định, thẻ form cuộn, 16 phím neo đáy, ✓ = lưu |
