...
6. ~~Call `db.walletDao.updateBalance(walletId, newWalletBalance)`.~~
   ⚠️ **Không còn đúng từ 2026-09-13** (G37): số dư ví suy từ sổ giao dịch, và
   `SoDuViService` là nơi duy nhất ghi `balance`. Đường nạp mục tiêu nay gọi
   `datNeoNhieuVi` trước khi ghi rồi `tinhLaiNhieuVi` sau khối nguyên tử.
