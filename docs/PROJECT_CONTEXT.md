...
Xem đầy đủ tại **`docs/CLIENT_APP_KNOWN_GAPS.md`**. Phiên 2026-09-03 đã đóng 9/10 mục còn mở; **G10 đóng ngày 2026-09-07**, và cùng ngày mở thêm **G23** (bản sao danh mục chỉ đầy đủ khi bộ mặc định *cục bộ* đầy đủ — tự khỏi ở lượt pull sau) và **G24** (màu danh mục không có cột trên server, chặn ở backend). **G15 đóng 2026-09-07** (dòng này từng còn liệt kê nó — soát lại 2026-09-09 từ chính `CLIENT_APP_KNOWN_GAPS.md`). Đếm lại từ chính `CLIENT_APP_KNOWN_GAPS.md` ngày 2026-09-10 — dòng cũ ở đây chỉ liệt kê ba mục và đã bỏ sót bốn: mục **chưa đóng** nay là **G18** (⏸️ thu hẹp dần), **G23** (⏸️ chấp nhận được), **G24** (⛔ chặn ở backend), **G26** (✅ cố ý — chờ màn duyệt giao dịch ngân hàng), **G27** (⏸️ hoãn có chủ ý — "ví được phép âm"), **G28** (⛔ chặn ở CSDL — lưu trữ ví chỉ sống trên máy đã bấm, mở 2026-09-10; sau khi gộp `main` cùng ngày thì phần mã backend đã xong, còn thiếu áp tệp `database/7`) , **G29** (⛔ chặn ở backend — `/sync/push` lọc nhầm `note` rồi bản đã lọc đè lên máy, mở 2026-09-10), **G30** (⏸️ chặn tạm ở client, chờ backend bỏ `uq_wallet_saving_active`, mở 2026-09-10), **G31** (⛔ chặn ở backend — tên dài hơn độ rộng cột rơi xuống `DB_ERROR` nên bị gửi lại mãi; client đã chặn ở bảy ô tên cùng ngày, còn chờ backend cho bản client cũ và Admin-web; mở 2026-09-10) và **G32** (⛔ chặn ở backend — `goal.Priority` `null` bị ghi thành `0`, mục tiêu chưa sắp nhảy lên đầu danh sách; client đọc `<= 0` là chưa sắp cùng ngày, còn chờ backend cho giá trị trên server và bản client cũ; mở 2026-09-10). **G19**, **G22** và **G25** ghi *không phải lỗi*, giữ lại để người sau không "sửa" nhầm:
