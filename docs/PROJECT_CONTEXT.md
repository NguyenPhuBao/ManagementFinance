...
- **Soát `7675b35` trên `main` — CAN-LAM 17** (2026-09-11, **không đổi mã client**, schema không đổi). Đầu phiên, `origin/main` đã có thêm `7675b35 fix backend 3` (NPBao, 23:15 ngày 10/09), gộp **sau** khi nhánh này vào `main` ở PR #71 — một commit tự nhận làm xong mọi tài liệu CAN-LAM, kèm `database/12`. Nó sửa đúng những tệp spec cưỡng chế đăng xuất dựa vào, nên spec chưa trình duyệt được. Nhánh này **chưa gộp** commit ấy; CSDL dev **chưa áp** tệp 12 (đo chỉ đọc: không cột nào của tệp 12, `uq_wallet_saving_active` còn, `chk_bill_pay_status` chưa có `'Skipped'`). `CAN-LAM/FIX_BACKEND_3_REGRESSIONS.md`.
  ...
  > **C — mã lỗi.** Ba mã mới (`BILL_ALREADY_PAID`, `WALLET_NAME_DUPLICATE`, `WALLET_DEFAULT_DUPLICATE`) không nằm trong `_permanentCodes`, nên gộp `main` là chúng rơi xuống `transient`; tài liệu backend lại ghi chúng "ánh xạ thành `CONSTRAINT_VIOLATION`". ⚠️ Hai mã `WALLET_*` là **client xin** (`WALLET_SAVING_INDEX.md` 4.2) — thêm chúng vào danh sách là việc của client, chưa làm.
