...
- **Số dư ví suy từ sổ giao dịch — đóng G37** (2026-09-13, **schema không đổi**, không thêm trường đồng bộ, **không xin backend gì**). Spec: `docs/superpowers/specs/2026-09-13-so-du-vi-suy-tu-so-giao-dich-design.md`. Mười task, mỗi task một commit, test đỏ trước.
  ...
  > **`SoDuViService` là nơi DUY NHẤT ghi `balance`**, thay cả 7 lời gọi `updateBalance` rải rác (bill 2, goal 4, transaction 1); test quét `lib/` thứ **năm** canh điều đó. **50 chỗ đọc `.balance` không sửa dòng nào** — đó là lý do chọn "cache của một công thức" thay vì "cột tính động".
