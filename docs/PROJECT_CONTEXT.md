...
- **Sửa G33 — tài khoản chờ xoá dùng tiếp 30 ngày** (2026-09-11, **schema không đổi**). Theo mục 4–5 (Phần 2–3, trừ §5.1) của `docs/superpowers/specs/2026-09-10-cuong-che-dang-xuat-va-cho-xoa-design.md`: `UserModel` mang `status`/`countdown`/`countdownNhanLuc`/`dangChoXoa`, gỡ `pendingDeleteCancelled` (`48b01c5`); repository ghi `PendingDelete` mà **không** xoá token (`d1791dd`); `AuthBloc` đọc lại thông tin tài khoản qua sự kiện mới `ThongTinTaiKhoanThayDoi` (`361f898` + `eee607e`); thẻ nhắc ở Trang chủ — nút "Để sau"/"Huỷ xoá" (`292af8d`); thẻ "Vùng nguy hiểm" ở Cài đặt — hộp đếm N ngày, nút "Huỷ yêu cầu xoá" (`f7a02aa` + `ff54c6b`); trang Xoá tài khoản thôi đăng xuất, thôi hứa "đăng nhập lại là tự khôi phục" (`866b870`); hàm đếm ngày thuần ở `ca44dd8`. Việc cho backend — `GET /auth/profile` trả thêm `countdown`, gỡ `pendingDeleteCancelled` khỏi response đăng nhập — đã viết thành CAN-LAM **19** (khối "Duyệt spec cưỡng chế đăng xuất..." trên). **Phần 1** (cưỡng chế đăng xuất, mục 3 kể cả §3.8) và **§5.1** (hộp thoại bị đẩy ra ở màn Đăng nhập) **chưa làm**.
  > **TDD:** cả bảy việc viết test trước; phần lớn đỏ đúng lý do ngay (thiếu tệp/hàm, hoặc câu chữ cũ "đăng nhập lại là khôi phục" / nút huỷ cũ còn hiện trên màn). Hai việc gặp một lượt đỏ **sai** lý do trước khi tới lượt đỏ đúng: repository (thiếu tham số `now` trên constructor `AuthRepositoryImpl`) và trang Xoá tài khoản (`GetIt` chưa nối `authRepository` truyền từ test) — cả hai là lỗi hạ tầng test, phải nối xong rồi mới đỏ đúng vì thiếu hành vi.
  > **Test và `analyze`:** `flutter test` toàn bộ **2073/2073 pass** (3 phút 4 giây; mức nền trước G33: 2029/2029) — 44 test mới ở 7 tệp (`dem_nguoc_xoa_test` 13, `user_model_cho_xoa_test` 5, `auth_repository_cho_xoa_test` 8, `auth_bloc_cho_xoa_test` 5, `the_cho_xoa_trang_chu_test` 6, `vung_nguy_hiem_card_test` 4, `delete_account_page_test` 3). `flutter analyze` **25 issue = 20 info + 5 warning + 0 error** — khớp mức nền, cùng năm tệp như trước, không issue nào ở tệp G33. Tệp test nay **199** `_test.dart` (**200** `.dart` kể cả `category_test_fakes.dart`) / **44.905** dòng — trước G33: 192 / 193 / 44.041.
