...
| **Financial Chatbot (Trợ lý ảo)** | Phase 1 $\rightarrow$ Phase 4 | • Function-Calling (truy vấn số liệu có cấu trúc)<br>• Semantic Chunking + HNSW cho kiến thức tĩnh<br>• HyDE + Hybrid Search + RRF<br>• Cross-Encoder Reranker<br>• Ragas Benchmark | **Kiến thức tài chính chung/tĩnh** (mẹo tiết kiệm, thuế TNCN, quy tắc 50/30/20). **Dữ liệu tài chính của User KHÔNG index lên server** (tuân thủ F1 & Nghị định 13/2023/NĐ-CP); số liệu cá nhân do Client cung cấp dưới dạng gói số đã tính hoặc truy vấn qua Function-Calling. |
