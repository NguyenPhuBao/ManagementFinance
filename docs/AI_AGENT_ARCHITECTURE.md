...
4. 🛑 **Lát 4b — tool layer + vòng lặp: Task 1–4 xong mã, DỪNG ở cổng Task 4 (2026-09-23).**
   Spike trên Realme: engine **sập native 3/3** khi phiên mang tool — `flutter_gemma_litertlm`
   1.7.0 gắn cứng giải mã có ràng buộc, và `CompositeLogitMask::Apply` sập ngay lượt giải mã đầu;
   đường bậc 1 cùng APK chạy bình thường (mục **9.13**, bẫy **4.33** `AI_EDGE_FEATURE.md`). Tầng
