...
> | **G48** | ⏸️ **Mở 2026-09-21** — đường tắt "Xem giao dịch" của màn Quản lý ví **thôi lọc sẵn ví** nếu tab Giao dịch đã mở trước đó trong phiên: `_filter` lấy `initialWalletId` trong **field initializer**, mà State của nhánh shell còn sống nên `initState` không chạy lại. Hỏng im lặng — người dùng thấy sổ đầy đủ và tưởng ví ấy có chừng ấy khoản. Máy ảo bắt được 2026-09-21; **không** phải hồi quy của lát cùng ngày, nó có từ nhóm D (2026-09-19). Chưa sửa vì ba lối sửa khác nhau ở chỗ *ai làm chủ bộ lọc* |
>
...
### G48 — Đường tắt "Xem giao dịch" thôi lọc sẵn ví nếu tab Giao dịch đã mở trước đó · ⏸️ MỞ (2026-09-21)
...
**Chưa sửa vì nó là một quyết định thiết kế, không phải một dòng sai.** Ít nhất
ba lối, và chúng khác nhau ở chỗ *ai làm chủ bộ lọc*: `didUpdateWidget` so
`initialWalletId` cũ/mới (rẻ nhất, nhưng phải định nghĩa "người dùng đã tự đổi
lọc rồi thì có đè không"); đọc lại query param mỗi lần route đổi; hoặc chuyển bộ
lọc lên bloc để nó là trạng thái điều hướng chứ không phải trạng thái widget.
Lối thứ ba dọn được cả chuyện bộ lọc sống sót qua lần ghé tab — thứ hiện chưa ai
chốt là nên hay không nên.

**Cách đi vòng hiện tại:** bấm chip Ví rồi chọn tay.
