...
- **`CAN-LAM/FIX_BACKEND_3_REGRESSIONS.md`** (2026-09-11) — ba hồi quy của `7675b35` trên `main`, **chưa gộp** về nhánh client: bắt tay socket và `/auth/refresh` từ chối mọi tài khoản; chốt trả hai lần đặt ở `upsertBill` chặn hoàn tác thanh toán; tài liệu backend ghi sai ba mã lỗi. Chưa mở G riêng vì chưa có gì chạy trên máy client. ⚠️ Nhưng **gộp `main` là hai chỗ phía client lộ ra ngay**: `WALLET_NAME_DUPLICATE` / `WALLET_DEFAULT_DUPLICATE` — hai mã client tự xin — chưa có trong `_permanentCodes` nên rơi xuống `transient`, và hoàn tác một hoá đơn đã đồng bộ kẹt hàng đợi đẩy cho tới khi backend sửa hồi quy B.
