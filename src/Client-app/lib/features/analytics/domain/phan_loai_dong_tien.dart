/// "Khoản này thuộc phân loại nào" — **định nghĩa duy nhất** — và phép gom
/// tổng của từng phân loại, nguồn cho ba chip của khối "Cơ cấu theo danh mục".
///
...
/// Tổng của một phân loại. Từng là một lát của vòng tròn "Cơ cấu dòng tiền"
/// (A8 #2, bỏ ngày 2026-09-14); nay nó nói nhóm ấy **có phát sinh hay không**,
/// tức khối "Cơ cấu theo danh mục" có hiện chip cho nó hay không.
class LatPhanLoai {
  /// Một trong `kCategoryClassifies`.
  final String phanLoai;
  final double soTien;

  /// Tỉ lệ trên tổng của **cả ba phân loại**, trong `[0, 1]`.
  final double tiLe;
...
/// Danh mục **bên trong** một nhóm, sắp giảm dần theo số tiền.
///
/// Tỉ lệ tính trên tổng **của nhóm**, không phải trên tổng cả ba: người dùng
/// đã chọn nhóm rồi, câu hỏi lúc này là "trong nhóm này, cái nào lớn".
///
/// Tổng các dòng trả về **bằng đúng** [LatPhanLoai.soTien] của cùng phân loại
/// — đó là điều giữ cho donut và danh sách cuối trang nói cùng một con số.
///
