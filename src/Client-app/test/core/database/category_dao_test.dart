...
void main() {
  ...
  test('migrates v2 category data and creates category keyword storage', () async {
    await db.close();
    ...
    final columns = await upgraded.customSelect('PRAGMA table_info(categories)').get();
  ...
  test('returns raw visible category rows from both category row APIs', () async {
    ...
    for (final rows in [watchedRows, fetchedRows]) {
      expect(rows.map((row) => row.id), containsAll(['global-food', 'personal-food']));
